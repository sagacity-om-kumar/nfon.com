...
Date        : 24-Jan-2020
Description :
- Uses as registering routers for apiExecutor Module.
...
func registerRouters(router *gin.RouterGroup) {
	logger.Log(helper.MODULENAME, logger.DEBUG, "Registering apiExecutor routes.")

	router.POST("/v1/apiexecutor/uploadtemplatedata", commandHandler)
...
func requestHandler(pContext *gin.Context) (bool, interface{}) {
	...
	case "/v1/apiexecutor/uploadtemplatedata":
		var reqData map[string]interface{}
		if isSuccess, resultData = ghelper.PrepareExecutionData(pContext, &reqData); !isSuccess {
			return false, resultData
		}
		isSuccess, resultData = APIExecutorService.UploadTemplateData(APIExecutorService{}, resultData.(*gModels.ServerActionExecuteProcess))
		break
