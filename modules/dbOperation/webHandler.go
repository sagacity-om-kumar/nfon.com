...
Date        : 24-Jan-2020
Description :
- Uses as registering routers for dbOperation Module.
...
func registerRouters(router *gin.RouterGroup) {
	logger.Log(helper.MODULENAME, logger.DEBUG, "Registering dbOperation routes.")

	router.POST("/v1/dboperation/uploadtemplatedata", commandHandler)
...
func requestHandler(pContext *gin.Context) (bool, interface{}) {
	...
	case "/v1/dboperation/uploadtemplatedata":
		var reqData map[string]interface{}
		if isSuccess, resultData = ghelper.PrepareExecutionData(pContext, &reqData); !isSuccess {
			return false, resultData
		}
		isSuccess, resultData = DBOperationService.UploadTemplateData(DBOperationService{}, resultData.(*gModels.ServerActionExecuteProcess))
		break
