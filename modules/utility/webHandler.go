...
Date        : 24-Jan-2020
Description :
- Uses as registering routers for Utility Module.
...
func registerRouters(router *gin.RouterGroup) {
	logger.Log(helper.MODULENAME, logger.DEBUG, "Registering utility routes.")

	router.POST("/v1/utility/updateappsettingvalue", commandHandler)
...
func requestHandler(pContext *gin.Context) (bool, interface{}) {
	...
	case "/v1/utility/updateappsettingvalue":
		reqData := gModels.AppSettingRequestDataModel{}
		if isSuccess, resultData = ghelper.PrepareExecutionData(pContext, &reqData); !isSuccess {
			return false, resultData
		}
		isSuccess, resultData = UtilityService.UpdateAppSettingValue(UtilityService{}, resultData.(*gModels.ServerActionExecuteProcess))
		break
